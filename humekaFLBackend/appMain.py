...
@app.route('/collect-cry', methods=['POST'])
def collect_cry_data():
    """Collect cry recording and expert annotation for research dataset"""
    try:
        if 'file' not in request.files:
        ...
        baby_weight = request.form.get('baby_weight')
        baby_gender = request.form.get('baby_gender')
        expert_assessment = request.form.get('expert_assessment')
        recording_duration = request.form.get('recording_duration')
        professional_id = request.form.get('professional_id')
        device_name = request.form.get('device_name', 'Mobile App')
        
        # Validate required fields
        if not all([baby_age, baby_weight, baby_gender, expert_assessment, recording_duration, professional_id]):
            return jsonify({'error': 'Missing required annotation data'}), 400
        
        # Save audio file
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"cry_{timestamp}_{secure_filename(file.filename)}"
        filepath = os.path.join('uploads', filename)
        file.save(filepath)
        
        # Convert audio for processing (if needed)
        processed_filepath = filepath
        if file.filename.endswith('.caf'):
            # Convert CAF to WAV for processing
            wav_filename = filename.replace('.caf', '.wav')
            wav_filepath = os.path.join('uploads', wav_filename)
            convert_audio(filepath, wav_filepath)
            processed_filepath = wav_filepath
        
        # Perform ML analysis
        try:
            ml_prediction = analyze_cry_audio(processed_filepath)
        ...
        try:
            ...
            cry_record = Cry(
                ...
                file_path=filepath,
                recording_device=device_name,
                cry_type=expert_assessment,
                expert_annotation=True,
                expert_assessment=expert_assessment,
                ml_prediction=ml_prediction,
                confidence_score=0.8,  # Default confidence
                id_professional=professional_id
            )
            ...
            return jsonify({
                'success': True,
                'message': 'Cry data collected successfully',
                'cry_record_id': cry_record.id_record,
                'ml_prediction': ml_prediction
            })
            ...
            db.session.rollback()
            return jsonify({'error': 'Failed to store in database'}), 500
            
    except Exception as e:
        print(f"Error collecting cry data: {e}")
